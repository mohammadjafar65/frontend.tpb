...
function Continents() {
  ...
  const handleAddOrEdit = async (data) => {
    if (editData) await updateContinent(editData.id, data);
    else await addContinent(data);
    setShowForm(false);
    setEditData(null);
    loadContinents();
  };
  ...
  const filtered = useMemo(() => {
    ...
    if (!k) return continents;
    return continents.filter((c) => String(c.name || "").toLowerCase().includes(k));
  }, [continents, query]);
  ...
  const handleBulkDelete = async () => {
    ...
    if (!window.confirm(`Delete ${ids.length} selected continent${ids.length === 1 ? "" : "s"}?`)) return;
    try {
      setBulkDeleting(true);
      await Promise.allSettled(ids.map((id) => deleteContinent(id)));
      clearSelection();
      loadContinents();
    } finally {
            ...
            {filtered.map((continent) => (
              <div key={continent.id} className="relative">
                <input
                  ...
                  checked={selectedIds.has(continent.id)}
                  onChange={() => toggleOne(continent.id)}
